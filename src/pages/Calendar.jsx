  ...
  return (
    <div className="w-full p-8 font-sans">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-3xl font-semibold">November <span className="font-light">2025</span></h1>
...
/* ========== SMALL COMPONENT FOR EACH DAY CELL ========== */
function CalendarDay({ number, events = [], dim = false, highlight = false, showMore = false }) {
  return (
    <div className={`h-32 p-2 border-r border-b relative ${dim ? "bg-gray-50 text-gray-400" : ""}`}>
      <div className="flex justify-between">
        <span className={`text-sm ${highlight ? "w-5 h-5 flex items-center justify-center rounded-full bg-black text-white font-bold" : ""}`}>
          {number}
        </span>
      </div>
      ...
      <div className="mt-1 space-y-1">
        {events.slice(0, 3).map((ev, idx) => (
          <div
            key={idx}
            className={`text-xs px-2 py-1 rounded-sm truncate ${ev.color}`}
          >
            {ev.label}
          </div>
        ))}
